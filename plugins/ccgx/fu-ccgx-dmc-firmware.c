...
{
	g_return_val_if_fail (FU_IS_CCGX_DMC_FIRMWARE (self), NULL);
	return self->custom_meta_record;
}
...
fu_ccgx_dmc_firmware_to_string (FuFirmware *firmware, guint idt, GString *str)
{
	...
	fu_common_string_append_kv (str, idt, "Version",
				    fu_ccgx_dmc_version_to_string (self->fwct_info.composite_version));
}

static gboolean
fu_ccgx_dmc_firmware_parse_segment (FuFirmware *firmware,
				const guint8 *fw_buff,
				gsize fw_buff_size,
				...
				GError **error)
{
	...
	for (guint32 seg_num = 0; seg_num < img_rcd->info_header.num_img_segments; seg_num++) {
		...
		g_autofree guint8 *row_buff = NULL;
		/* alloc segment record */
		...
		if (!fu_memcpy_safe ((guint8 *)&seg_rcd->info_header, sizeof(seg_rcd->info_header), 0x0, /* dst */
			     fw_buff, fw_buff_size, fw_buffer_offset, sizeof(seg_rcd->info_header), error)) /* src */
			return FALSE;
		/* calculate actual row size */
		actual_row_size = img_rcd->info_header.row_size * 64;
		/* alloc row buffer */
		row_buff = g_malloc0 (actual_row_size);
		...
		for (int row = 0; row < seg_rcd->info_header.num_rows; row++) {
			...
			/* read row data */
			if (!fu_memcpy_safe (row_buff, actual_row_size, 0x0, /* dst */
			     fw_buff, fw_buff_size, fw_buffer_offset, actual_row_size, error)) /* src */
				return FALSE;
			/* update hash */
			g_checksum_update (csum, (guchar *) row_buff, actual_row_size);
			/* add row data to data record */
			data_rcd->data = g_bytes_new (row_buff, actual_row_size);
				...
				GError **error)
{
	FuCcgxDmcFirmware *self = FU_CCGX_DMC_FIRMWARE (firmware);
	gsize fw_buffer_offset = 0;
	gsize img_info_offset = 0;
	gsize seg_info_offset = 0;

	/* set initial image info offset */
	img_info_offset = sizeof(FwctInfo);
	...
	for (guint32 img_num = 0; img_num < self->fwct_info.image_count; img_num++) {
		...
		if (!fu_memcpy_safe ((guint8 *)&img_rcd->info_header, sizeof(img_rcd->info_header), 0x0, /* dst */
			     fw_buff, fw_buff_size, fw_buffer_offset, sizeof(img_rcd->info_header), error)) /* src */
			return FALSE;
		...
		/* parse segment */
		if (!fu_ccgx_dmc_firmware_parse_segment (firmware, fw_buff, fw_buff_size, img_rcd,
...
{
	FuCcgxDmcFirmware *self = FU_CCGX_DMC_FIRMWARE (firmware);
	gsize fw_buff_size = 0;
	const guint8 *fw_buff = g_bytes_get_data (fw, &fw_buff_size);
	guint16 custom_meta_size = 0;
	g_autoptr(FuFirmwareImage) img = fu_firmware_image_new (fw);
	g_autofree guint8 *fwct_buff = NULL;
	g_autofree guint8 *custom_meta_buff = NULL;

	/* read fwct info */
	if (!fu_memcpy_safe ((guint8 *) &self->fwct_info, sizeof(self->fwct_info), 0x0, /* dst */
			     fw_buff, fw_buff_size, 0, sizeof(self->fwct_info), error)) /* src */
		return FALSE;
	...
	if (self->fwct_info.signature != DMC_FWCT_SIGN) {
		g_set_error (error,
			     ...
			     (guint32)DMC_FWCT_SIGN, (guint32)self->fwct_info.signature);
		return FALSE;
	}
	/* check fwct size */
	if (self->fwct_info.size > DMC_FWCT_MAX_SIZE || self->fwct_info.size == 0) {
		g_set_error (error,
			     FWUPD_ERROR,
			     FWUPD_ERROR_NOT_SUPPORTED,
			     "invalid dmc fwct size, expected equal to or less than %u, got %u",
			     (guint32)DMC_FWCT_MAX_SIZE,  (guint32)self->fwct_info.size);
		return FALSE;
	}
	/* alloc fwct buffer */
	fwct_buff = g_malloc0 (self->fwct_info.size);
	/* read fwct data */
	if (!fu_memcpy_safe ((guint8 *)fwct_buff, self->fwct_info.size, 0x0, /* dst */
		     fw_buff, fw_buff_size, 0, self->fwct_info.size, error)) /* src */
		return FALSE;
	/* create fwct binary */
	self->fwct_record = g_bytes_new (fwct_buff, self->fwct_info.size);
	/* create custom meta binary */
	if (!fu_common_read_uint16_safe (fw_buff, fw_buff_size, self->fwct_info.size, &custom_meta_size,
			    G_LITTLE_ENDIAN,error))
		return FALSE;
	if (custom_meta_size > 0) {
		/* alloc custom meta buffer */
		custom_meta_buff = g_malloc0 (custom_meta_size);
		/* read custom meta data */
		if (!fu_memcpy_safe ((guint8 *)custom_meta_buff, custom_meta_size, 0x0, /* dst */
			     fw_buff, fw_buff_size, self->fwct_info.size + 2, custom_meta_size, error)) /* src */
			return FALSE;
		self->custom_meta_record = g_bytes_new (custom_meta_buff, custom_meta_size);
	}

	/* set row data start offset */
	self->row_data_offset_start = self->fwct_info.size + DMC_CUSTOM_META_LENGTH_FIELD_SIZE + custom_meta_size;
	self->fw_data_size = fw_buff_size - self->row_data_offset_start;
	/* parse image */
	if (!fu_ccgx_dmc_firmware_parse_image (firmware, fw_buff, fw_buff_size, error))
		return FALSE;

	/* add something, although we'll use the records for the update */
	fu_firmware_image_set_addr (img, 0);
...
fu_ccgx_dmc_firmware_finalize (GObject *object)
{
	...
	if (self->fwct_record)
		g_bytes_unref (self->fwct_record);
	if (self->custom_meta_record)
		g_bytes_unref (self->custom_meta_record);
