...
#define DMC_FW_WRITE_STATUS_RETRY_DELAY_MS 30 /* 30 msec */
			        ...
			        GError **error)
{
	...
	if (!g_usb_device_control_transfer (fu_usb_device_get_dev (FU_USB_DEVICE (self)),
					    ...
					    NULL, /* actual length */
					    DMC_CONTROL_TRANSFER_DEFAULT_TIMEOUT, /* timeout */
					    NULL, error)) {
...
fu_ccgx_dmc_device_send_start_upgrade (FuCcgxDmcDevice *self,
				       guint8* custom_meta_data,
				       guint16 custom_meta_size,
				       GError **error)
{
	guint16 value = 0;
	if (custom_meta_size > 0)
		value = 1;
	if (custom_meta_data == NULL && custom_meta_size > 0) {
		g_set_error(error,
			     FWUPD_ERROR,
			     FWUPD_ERROR_NOT_SUPPORTED,
			     "invalid  pointer at %s",__func__);
	...
	if (!g_usb_device_control_transfer (fu_usb_device_get_dev (FU_USB_DEVICE (self)),
					    ...
					    custom_meta_data, /* data */
					    custom_meta_size,  /* length */
					    NULL, /* actual length */
					    DMC_CONTROL_TRANSFER_DEFAULT_TIMEOUT, /* timeout */
					    NULL, error)) {
		g_prefix_error (error, "send reset error:");
				  ...
				  GError **error)
{
	...
	if (!g_usb_device_interrupt_transfer (fu_usb_device_get_dev (FU_USB_DEVICE (self)),
					      self->ep_intr_in,
					      (guint8*)intr_rqt, sizeof(DmcIntRqt), NULL,
...
static gboolean
fu_ccgx_hpi_write_firmware_image (FuDevice *device,
				  FuCcgxDmcFirmwareImageRecord *img_rcd,
				  GError **error)
{
	FuCcgxDmcDevice *self = FU_CCGX_DMC_DEVICE (device);
	GPtrArray *seg_records = NULL;
	...
	for (guint32 seg_index = 0; seg_index < seg_records->len; seg_index++) {
		...
		if (!fu_ccgx_dmc_device_send_write_command (self,seg_rcd->info_header.start_row,
		...
		for (guint32 data_index = 0; data_index < data_records->len; data_index++) {
			guint8* row_buffer = NULL;
			gsize row_size = 0;
			/* get data from records */
			FuCcgxDmcFirmwareDataRecord *data_rcd = g_ptr_array_index (data_records, data_index);
			/* get row buffer from data */
			...
			self->fw_data_written += row_size;
			/* update update progress */
			fu_device_set_progress_full (device, (gsize) self->fw_data_written, (gsize) self->fw_data_size);
			/* get reponse  */
			for (guint8 retry = 0; retry < 2; retry++) {
				g_autoptr(GError) error_local = NULL;
				/* get interrupt request */
				if (!fu_ccgx_dmc_device_read_intr_req (self, &dmc_int_req, &error_local)) {
					g_set_error (error,
						     FWUPD_ERROR,
						     FWUPD_ERROR_INTERNAL,
						     "failed read intr req: %s",
						     error_local->message);
				       return FALSE;
				}
				...
				if (dmc_int_req.opcode != DMC_INT_OPCODE_IMG_WRITE_STATUS) {
					g_set_error (error,
						     FWUPD_ERROR,
						     FWUPD_ERROR_NOT_SUPPORTED,
						     "invalid dmc intr req opcode in fw write = %d",
						     dmc_int_req.opcode);
						return FALSE;
				}
				if (dmc_int_req.data[0] == 0) /* success */
					break;
				else if (dmc_int_req.data[0] != 1)  {/* retry if data[0] is 1 otherwise error */
					g_set_error (error,
						     FWUPD_ERROR,
						     FWUPD_ERROR_NOT_SUPPORTED,
						     "invalid dmc intr req data in fw write = %d",
						     dmc_int_req.data[0]);
						return FALSE;
				}
...
{
	FuCcgxDmcDevice *self = FU_CCGX_DMC_DEVICE (device);
	GBytes *fwct_record = NULL;
	GBytes *custom_meta_record = NULL;
	GPtrArray *image_records = NULL;
	const guint8* custom_meta_data = NULL;
	gsize custom_meta_size = 0;
	const guint8* fwct_data = NULL;
	gsize fwct_size = 0;
	...
	/* get fwct record */
	fwct_record = fu_ccgx_dmc_firmware_get_fwct_record (FU_CCGX_DMC_FIRMWARE(firmware));
	/* get custom meta record */
	custom_meta_record = fu_ccgx_dmc_firmware_get_custom_meta_record (FU_CCGX_DMC_FIRMWARE(firmware));
	/* get image records */
	image_records = fu_ccgx_dmc_firmware_get_image_records (FU_CCGX_DMC_FIRMWARE(firmware));

	if (custom_meta_record)
		custom_meta_data = g_bytes_get_data (custom_meta_record, &custom_meta_size);

	g_debug ("reset dmc state machine");
	if (!fu_ccgx_dmc_device_send_reset_state_machine (self, error))
		return FALSE;

	/* start fw upgrade with custom meta data */
	if (!fu_ccgx_dmc_device_send_start_upgrade (self, custom_meta_data, custom_meta_size, error))
		return FALSE;

	fwct_data = g_bytes_get_data (fwct_record, &fwct_size);
	if (fwct_data == NULL) {
	...
	/* send fwct data */
	g_debug ("fw write request with meta:%u fwct:%u", (guint32)custom_meta_size, (guint32)fwct_size);
	if (!fu_ccgx_dmc_device_send_fwct (self,fwct_data, fwct_size, error))
		return FALSE;


	/* init fw written */
	self->fw_data_written = 0;
	/* get total fw size */
	self->fw_data_size = fu_ccgx_dmc_firmware_get_fw_data_size (FU_CCGX_DMC_FIRMWARE(firmware));
	g_debug ("updating fw...");
	while (1) {
		...
		if (dmc_int_rqt.opcode == DMC_INT_OPCODE_FW_UPGRADE_RQT) {
			...
			g_debug ("write image #%d",img_index);
			...
			/* write image */
			if (!fu_ccgx_hpi_write_firmware_image (device, img_rcd, error))
				return FALSE;
		} else
			break;
	}
	...
	if (dmc_int_rqt.data[0] == DMC_DEVICE_STATUS_UPDATE_PHASE_1_COMPLETE)
		self->update_model = DMC_UPDATE_MODEL_DOWNLOAD_TIRGGER;
				     ...
				     GError **error)
{
	...
	custom_meta_record = fu_ccgx_dmc_firmware_get_custom_meta_record (FU_CCGX_DMC_FIRMWARE(firmware));
	if (custom_meta_record)
		if (g_bytes_get_size (custom_meta_record) > 0)
...
fu_ccgx_dmc_device_attach (FuDevice *device, GError **error)
{
	...
	if (self->update_model == DMC_UPDATE_MODEL_DOWNLOAD_TIRGGER) {
		...
		g_debug ("pending reset:%u",(guint32)manual_replug);
	...
	g_debug ("reset and wait replug");

	remove_delay_ms = fu_device_get_install_duration(self) * 1000;
	fu_device_set_remove_delay (FU_DEVICE (self), remove_delay_ms); /* over a minute to flash offline */
...
fu_ccgx_dmc_device_setup (FuDevice *device, GError **error)
{
	...
	if (!fu_memcpy_safe ((guint8 *)&self->dock_id, sizeof(DmcDockIdentity), 0x0, /* dst */
		     (guint8*)&dock_id, sizeof(DmcDockIdentity), 0, sizeof(DmcDockIdentity), error)) /* src */
		return FALSE;
	...
	/* set instance id */
	instance_id = g_strdup_printf ("USB\\VID_%04X&PID_%04X&DMC",
	...
	version = fu_ccgx_dmc_version_to_string (version_raw);
	fu_device_set_version (FU_DEVICE (self), version);
	fu_device_set_version_raw (FU_DEVICE (self), version_raw);

	g_debug  ("[dmc dock] VID:%04X, PID:%04X, model:%d, vendor:%s, product:%s",
		dock_id.vid, dock_id.pid, dock_id.model, dock_id.vendor_string, dock_id.product_string);

	g_debug  ("[dmc status] status:0x%X, device count:%d, version:%s",
		dock_status.device_status, dock_status.device_count, version);
